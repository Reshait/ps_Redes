...
#define MSG_SIZE 270
#define MAX_USUARIOS_X_MESA 2
#define NUMPARTIDAS 2
#define MAX_CLIENTS MAX_USUARIOS_X_MESA*NUMPARTIDAS
    ...
    /*
        LOS ESTADOS POSIBLES SON:
               -1 - USUARIO INACTIVO EN EL SISTEMA 
                ...
                2 - USUARIO LOGUEADO EN EL SISTEMA
                3 - ESPERANDO PARTIDA
                4 - JUGANDO
    */
    int numSock;
...
void FALSO(){
  ...
  const void* vals[] = { CFSTR("MENTIROSO!!!"), CFSTR("Usted NO tiene ninguna línea en su cartón\n")};
  ...
  CFUserNotificationCreate(kCFAllocatorDefault, 0, kCFUserNotificationPlainAlertLevel, 0, dict);
}

void LINEA(){
  const void* keys[] = { kCFUserNotificationAlertHeaderKey, kCFUserNotificationAlertMessageKey };
  const void* vals[] = { CFSTR("PREMIO!!!"), CFSTR("HA CONSEGUIDO EL PREMIO DE UNA LÍNEA\n")};
...
void LINEA2(){
  const void* keys[] = { kCFUserNotificationAlertHeaderKey, kCFUserNotificationAlertMessageKey };
  const void* vals[] = { CFSTR("PREMIO!!!!!!"), CFSTR("HA CONSEGUIDO EL PREMIO DE DOS LÍNEAS\n")};
...
void BINGO(){
  const void* keys[] = { kCFUserNotificationAlertHeaderKey, kCFUserNotificationAlertMessageKey };
  const void* vals[] = { CFSTR("********BINGO********"), CFSTR("¡¡¡¡HA CONSEGUIDO EL PREMIO DEL BINGO!!!!\n")};
...
{       
        ...
        if(f)
        {
                while(fscanf(f,"%s %s",A.nick,A.pass)==2)
                {
...
int lineasEnCarton(int carton [][9], int sorteo[90])
{
	int i, j, k, numerosEnLinea, lineas = 0;
  for(i = 0; i < 3; i++){
    numerosEnLinea = 0;
    for(j = 0; j < 9; j++){
        if(sorteo[carton[i][j]] == 1){
          numerosEnLinea++;
          if(numerosEnLinea == 5)
            lineas++;
        }
